...
public class GourmetCoffee  {
	...
	public void displayCatalog() {
		...
		if (size == 0) {
			...
			for (i=0;i<=catalog.getNumberOfProducts()-1;i++) {

				stdOut.println(catalog.products.get(i).getCode() + " " +
						catalog.products.get(i).getDescription());
			}
		}
	}
	...
	public void displayOrder() {
		...
		if (size == 0) {
			...
			for (i=0;i<=currentOrder.getNumberOfItems()-1;i++) {
				stdOut.println(currentOrder.items.get(i).toString());
			}
	...
	public void displayOrdersSold() {
		...
		if (numOrders != 0) {
			int orderNumber = 1;
			for (i = 0;i <= sales.getNumberOfOrders() - 1;i++) {

				stdOut.println("Order " + orderNumber++);

				for (j = 0;j <= currentOrder.getNumberOfItems()-1;j++) {
					stdOut.println("   " + sales.orders.get(j).toString());
				}
				stdOut.println("   Total: " +
					CURRENCY.format(sales.orders.get(i).getTotalCost()));
			}
		} else {
	...
	public void displayNumberOfOrders(Product product) {
		...
		int s=0;
		for(i = 0;i <= sales.getNumberOfOrders() - 1;i++){
			for(j = 0;j <= sales.orders.get(i).items.size() - 1;j++){
				if(sales.orders.get(i).items.get(j).getProduct().equals(product))
					s++;
			}
		}
		stdErr.println("NumberOfOrders:" + s);
	...
	public void displayTotalQuantityOfProducts() {
		int i,j,k;
		int[] a = new int[catalog.getNumberOfProducts()];
		  for(k = 0;k <= catalog.getNumberOfProducts() - 1;k++){
	        	a[k] = 0;
	        }
		for(i = 0;i <= sales.getNumberOfOrders() - 1;i++){
			for(j = 0;j <= sales.orders.get(i).items.size() - 1;j++){
				for(k = 0;k <= catalog.getNumberOfProducts() - 1;k++){
					if(sales.orders.get(i).items.get(j).getProduct().getCode().equals(catalog.products.get(k).getCode()))
						a[k] = a[k] + sales.orders.get(i).items.get(j).getQuantity();
				}
